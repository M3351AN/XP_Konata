// fix VC6 build
    ...
    #define _wfopen_s(pFile, filename, mode) \
        (((*(pFile)) = _wfopen((filename), (mode))) == NULL)

    #define _wfreopen_s(pFile, filename, mode, stream) \
        (((*(pFile)) = _wfreopen((filename), (mode), (stream))) == NULL)

    #define localtime_s(tp, time) \
        ((*(tp)) = *localtime((time)), 0)
